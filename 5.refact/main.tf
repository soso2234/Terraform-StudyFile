...
resource "aws_eip" "tf_nat_eip" {
  tags = { Name = "${var.pjt_name}-nat-eip" }
}
...
resource "aws_instance" "tf_ec2" {
  ...
  tags = { Name = "${var.pjt_name}-ec2-${count.index + 1}" }
}
